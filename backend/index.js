...
if (!uri || uri.includes("your_mongoDB_url_here")) {
  uri = "mongodb://127.0.0.1:27017/tradeX";
}
...
app.get("/allHoldings", async (req, res) => {
  try {
    if (mongoose.connection.readyState === 1) {
      ...
      if (allHoldings.length > 0) return res.json(allHoldings);
    }
  } catch (err) {
    console.log("DB query failed, serving mock data");
  }
...
app.get("/allPositions", async (req, res) => {
  try {
    if (mongoose.connection.readyState === 1) {
      ...
      if (allPositions.length > 0) return res.json(allPositions);
    }
  } catch (err) {
    console.log("DB query failed, serving mock data");
  }
...
app.listen(PORT, () => {
  ...
  mongoose.connect(uri)
    ...
    .catch((err) => {
      console.error("DB failed to start (MongoDB connection issue):", err.message);
      console.log("Tip: If you don't have MongoDB running locally, the app will continue to run, but data operations might fail.");
    });
});
